...
contract MarketCreator is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        address _raacToken,
        address _decrvUSDToken
    ) Ownable(initialOwner) {
        raacToken = IERC20(_raacToken);
        decrvUSDToken = IERC20(_decrvUSDToken);
    }

    // researcher -- join reseach market ----- request to join --- create his own stuff - requestion
    // name === onlyfans --- create -- strct of info(bond, period)
    ...
    function createMarket(
        ...
        if (_lockDuration == 0 || _lockDuration > MAX_LOCK_DURATION)
    ...
    function participateInMarket(
        ...
        require(
            market.quoteAsset != IERC20(address(0)),
        ...
        if (position.exists) {
            ...
            userPositions[marketId][msg.sender] = UserPosition(
                amount,
                block.timestamp + market.lockDuration,
                true
            );
        }
    ...
    function redeemFromMarket(uint256 marketId) external nonReentrant {
        ...
        require(
            block.timestamp >= position.lockEndTime,
            "Lock duration has not passed"
        );
    ...
    function getMarketInfo(
        uint256 marketId
    ) external view returns (IERC20, uint256, uint256, uint256) {
        Market storage market = markets[marketId];
        return (
            market.quoteAsset,
            market.lockDuration,
